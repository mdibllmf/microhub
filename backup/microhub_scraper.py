...
ORGANISM_KEYWORDS = {
    # All canonical names are scientific (Latin binomial) names
    'Mus musculus': [r'mus\s*musculus', r'm\.\s*musculus'],
    'Homo sapiens': [r'homo\s*sapiens', r'h\.\s*sapiens'],
    'Rattus norvegicus': [r'rattus\s*norvegicus', r'r\.\s*norvegicus'],
    'Danio rerio': [r'danio\s*rerio', r'd\.\s*rerio'],
    'Drosophila melanogaster': [r'drosophila\s*melanogaster', r'd\.\s*melanogaster', r'\bdrosophila\b'],
    'Caenorhabditis elegans': [r'caenorhabditis\s*elegans', r'c\.\s*elegans', r'caenorhabditis', r'c\.\s*briggsae'],
    'Xenopus laevis': [r'xenopus\s*laevis', r'x\.\s*laevis'],
    'Xenopus tropicalis': [r'xenopus\s*tropicalis', r'x\.\s*tropicalis'],
    'Gallus gallus': [r'gallus\s*gallus', r'g\.\s*gallus'],
    'Sus scrofa': [r'sus\s*scrofa', r's\.\s*scrofa'],
    'Macaca mulatta': [r'macaca\s*mulatta', r'm\.\s*mulatta'],
    'Macaca fascicularis': [r'macaca\s*fascicularis', r'm\.\s*fascicularis'],
    'Oryctolagus cuniculus': [r'oryctolagus\s*cuniculus', r'o\.\s*cuniculus'],
    'Canis lupus familiaris': [r'canis\s*familiaris', r'canis\s*lupus\s*familiaris', r'c\.\s*familiaris'],
    'Saccharomyces cerevisiae': [r'saccharomyces\s*cerevisiae', r's\.\s*cerevisiae'],
    'Schizosaccharomyces pombe': [r'schizosaccharomyces\s*pombe', r's\.\s*pombe'],
    'Escherichia coli': [r'escherichia\s*coli', r'e\.\s*coli'],
    'Arabidopsis thaliana': [r'arabidopsis\s*thaliana', r'a\.\s*thaliana', r'\barabidopsis\b'],
    'Nicotiana tabacum': [r'nicotiana\s*tabacum', r'n\.\s*tabacum'],
    'Nicotiana benthamiana': [r'nicotiana\s*benthamiana', r'n\.\s*benthamiana'],
    'Zea mays': [r'zea\s*mays', r'z\.\s*mays'],
    'Oryza sativa': [r'oryza\s*sativa', r'o\.\s*sativa'],
}
...
CELL_LINE_KEYWORDS = {
    # All canonical names are full names, no acronyms
    'HeLa (Henrietta Lacks)': [r'\bhela\b'],
    'Human Embryonic Kidney 293': ['hek293', 'hek-293', 'hek 293'],
    'Human Embryonic Kidney 293T': ['hek293t', '293t'],
    'Human Osteosarcoma U2OS': ['u2os', 'u-2 os'],
    'African Green Monkey Kidney COS-7': ['cos-7', 'cos7'],
    'Chinese Hamster Ovary': [r'\bcho\b.*cell'],
    'NIH 3T3 Mouse Fibroblast': ['nih 3t3', 'nih3t3', '3t3'],
    'Human Breast Adenocarcinoma MCF-7': ['mcf7', 'mcf-7'],
    'Human Lung Adenocarcinoma A549': ['a549'],
    'Madin-Darby Canine Kidney': ['mdck'],
    'Vero (African Green Monkey Kidney)': [r'\bvero\b'],
    'Rat Pheochromocytoma PC-12': ['pc12', 'pc-12'],
    'Human Neuroblastoma SH-SY5Y': ['sh-sy5y', 'shsy5y'],
    'Induced Pluripotent Stem Cell': ['ipsc', 'induced pluripotent'],
    'Embryonic Stem Cell': [r'\besc\b', 'embryonic stem'],
    'Mouse Embryonic Fibroblast': [r'\bmef\b', 'mouse embryonic fibroblast'],
...
Use empty arrays [] for categories with no applicable tags."""
    ...
    def calculate_priority(self, paper: Dict) -> int:
        ...
        # Abstract quality (proxy for content richness; tags added in step 3)
    ...
    def _parse_article(self, article, fetch_full_text: bool = True, fetch_cites: bool = True) -> Optional[Dict]:
        ...
        try:
            ...
            # Combined text for structured identifier extraction
            # (tag extraction is deferred to 3_clean.py)
            extraction_text = f"{title} {abstract} {full_methods} {full_text}"

            # ---- Tags: empty — tagging happens in step 3 (3_clean.py) ----
            microscopy_techniques = []
            microscope_brands = []
            reagent_suppliers = []
            microscope_models = []
            ...
            # ---- Structured identifiers: still extracted here ----
            # (these are URL/ID lookups, not classification)
            ...
            # Extract ALL GitHub URLs for tool tracking
            ...
            # LLM enrichment removed — tagging happens in step 3

            # Relevance check: paper must have at least a title
            # (all papers come from microscopy-specific PubMed queries,
            #  so they're already relevant — tagging happens in step 3)
            if not title or len(title.strip()) < 10:
                self.stats['skipped_no_tags'] += 1
                logger.debug(f"Skipping paper {pmid}: no title")
                return None
            ...
            # antibodies extracted in step 3
            if affiliations:
    ...
    def run(self, limit: int = None, priority_only: bool = False,
        ...
        logger.info("=" * 70)
        logger.info("MICROHUB PAPER SCRAPER v6.0 - COLLECT + ACQUIRE")
        logger.info("=" * 70)
        logger.info("Features: Full text + CITATIONS + AFFILIATIONS + identifiers")
        logger.info("Tagging deferred to step 3 (3_clean.py)")
        if self.ncbi_api_key:
        ...
        # Final stats
        logger.info("\n" + "=" * 70)
        logger.info("SCRAPING COMPLETE - v6.0 COLLECT + ACQUIRE")
        logger.info("=" * 70)
        logger.info(f"Queries run: {query_count}")
        logger.info(f"Papers found: {self.stats['found']:,}")
        logger.info(f"Duplicates skipped: {self.stats['duplicates_skipped']:,}")
        logger.info(f"Skipped (no title): {self.stats['skipped_no_tags']:,}")
