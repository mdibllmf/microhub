...
function microhub_import_paper($data, $skip_existing, $update_existing, &$enrichment_stats) {
    ...
    // Save facility info and set as taxonomy
    $facility = '';
    $facilities = array();
    $facility_url = '';
    ...
    // Get facilities array if present
    ...
    // Save all facilities as JSON and set taxonomy
    if (!empty($facilities)) {
        update_post_meta($post_id, '_mh_facilities', wp_json_encode($facilities));
