...
function microhub_process_import() {
    ...
    echo sprintf(__('Protocol URLs: %d, Protocol Papers: %d, Repositories: %d, GitHub: %d, Facilities: %d, Affiliations: %d, RRIDs: %d, RORs: %d, Microscopes: %d', 'microhub'),
        $enrichment_stats['protocols'], $enrichment_stats['protocol_papers'] ?? 0, $enrichment_stats['repositories'],
        $enrichment_stats['github'] ?? 0, $enrichment_stats['facilities'] ?? 0, $enrichment_stats['affiliations'] ?? 0,
...
function microhub_import_paper($data, $skip_existing, $update_existing, &$enrichment_stats) {
    ...
    if ($doi) {
        $doi_url = !empty($data['doi_url']) ? $data['doi_url'] : 'https://doi.org/' . $doi;
        update_post_meta($post_id, '_mh_doi_url', esc_url_raw($doi_url));
    }

    // Save PMID and generate PubMed URL
    $pmid = isset($data['pmid']) ? sanitize_text_field($data['pmid']) : '';
    update_post_meta($post_id, '_mh_pubmed_id', $pmid);
    if ($pmid) {
        $pubmed_url = !empty($data['pubmed_url']) ? $data['pubmed_url'] : 'https://pubmed.ncbi.nlm.nih.gov/' . $pmid . '/';
        update_post_meta($post_id, '_mh_pubmed_url', esc_url_raw($pubmed_url));
    }
    ...
    if (!empty($data['pmc_id'])) {
        $pmc_id = sanitize_text_field($data['pmc_id']);
        update_post_meta($post_id, '_mh_pmc_id', $pmc_id);
        // Generate and store PMC URL if not provided
        if (!empty($data['pmc_url'])) {
            update_post_meta($post_id, '_mh_pmc_url', esc_url_raw($data['pmc_url']));
        } else {
            // Ensure PMC ID has correct format
            $pmc_clean = str_replace('PMC', '', strtoupper($pmc_id));
            update_post_meta($post_id, '_mh_pmc_url', 'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC' . $pmc_clean . '/');
        }
    }
