...
function mh_theme_search_papers($request) {
    ...
    // Has protocols filter - check for non-empty value
    if ($request->get_param('has_protocols')) {
        $meta_query[] = array(
            'key' => '_mh_protocols',
            'value' => '%:%',  // JSON with any key:value
            'compare' => 'LIKE'
        );
        $has_meta_filter = true;
    }
    ...
    // Has RRIDs filter - check for non-empty value
    ...
    // Has data repositories filter - check for non-empty value
...
// ============================================
// GEMINI CHAT - USES PLUGIN'S REST API
// ============================================
// The theme uses the plugin's existing Gemini implementation
// API endpoint: /wp-json/microhub/v1/ai-chat
// API key is configured in MicroHub plugin settings (MicroHub → Settings)
