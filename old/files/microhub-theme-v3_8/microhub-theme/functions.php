...
function mh_get_paper_tags($post_id = null) {
    ...
    $microscope_brands = json_decode(get_post_meta($post_id, '_mh_microscope_brands', true), true) ?: array();
    foreach ($microscope_brands as $brand) {
        $tags[] = array(
            'name' => $brand,
            'slug' => sanitize_title($brand),
            'taxonomy' => 'microscope_brand',
            'url' => null
        );
    }
    
    // Get microscope models from meta
    $microscope_models = json_decode(get_post_meta($post_id, '_mh_microscope_models', true), true) ?: array();
    foreach ($microscope_models as $model) {
        $tags[] = array(
            'name' => $model,
            'slug' => sanitize_title($model),
            'taxonomy' => 'microscope_model',
            'url' => null
        );
    }
    ...
    // Get antibodies from meta
    $antibodies = json_decode(get_post_meta($post_id, '_mh_antibodies', true), true) ?: array();
