...
<section class="mh-results-section">
    <div class="mh-container">
        ...
        <div class="mh-main-layout">
            ...
            // Get facilities
            $facilities = mh_get_facilities(10);
            ?>
                ...
                <div class="mh-sidebar-widget">
                    ...
                    <ul class="mh-facility-list">
                        <?php foreach ($facilities as $facility) : 
                            $facility_slug = isset($facility['slug']) ? $facility['slug'] : sanitize_title($facility['name']);
                        ?>
                        <li class="mh-facility-item">
                            <a href="#" class="facility-link" data-facility="<?php echo esc_attr($facility_slug); ?>" onclick="filterByFacility('<?php echo esc_js($facility_slug); ?>'); return false;">
                                <?php echo esc_html(wp_trim_words($facility['name'], 6)); ?>
                            </a>
                            <?php if (!empty($facility['website'])) : ?>
                            <a href="<?php echo esc_url($facility['website']); ?>" class="facility-website" target="_blank" title="Visit facility website">🔗</a>
                            <?php endif; ?>
                            <?php if ($facility['count'] > 0) : ?>
                            <span class="count">(<?php echo $facility['count']; ?> papers)</span>
                            <?php endif; ?>
                        </li>
                        <?php endforeach; ?>
                    </ul>
                    <?php else : ?>
                    <p class="mh-empty-item">No facilities listed</p>
                    <?php endif; ?>
                </div>
            </aside>
        </div>
...
.mh-facility-item:last-child {
    border-bottom: none;
}

.mh-protocol-item .protocol-link,
.mh-facility-item .facility-link {
    display: block;
    color: var(--text);
...
.mh-facility-item .facility-link:hover {
    color: var(--primary);
}

.mh-facility-item .facility-link.active {
    color: var(--primary);
    font-weight: 600;
}
...
.mh-facility-item .facility-website {
    font-size: 0.75rem;
    text-decoration: none;
    opacity: 0.6;
...
.mh-facility-item .facility-website:hover {
    opacity: 1;
}

.mh-protocol-item .source,
.mh-facility-item .count {
    display: block;
    font-size: 0.75rem;
    color: var(--text-muted);
    margin-top: 2px;
...
<script>
// Global function for facility filtering (called from sidebar links)
function filterByFacility(facilitySlug) {
    // Access the activeFilters via the IIFE scope workaround
    if (window.mhFilterByFacility) {
        window.mhFilterByFacility(facilitySlug);
    }
}
...
(function() {
    ...
    // Expose facility filter function globally
    window.mhFilterByFacility = function(facilitySlug) {
        // Clear other filters but keep facility
        activeFilters = { facility: facilitySlug };
        currentPage = 1;
        ...
        // Highlight the active facility in sidebar
        document.querySelectorAll('.facility-link').forEach(link => {
            link.classList.remove('active');
            if (link.dataset.facility === facilitySlug) {
    ...
    document.querySelectorAll('.mh-quick-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (btn.classList.contains('active')) {
                ...
                delete activeFilters.has_full_text;
    ...
    clearBtn.addEventListener('click', () => {
        ...
        document.querySelectorAll('.facility-link').forEach(link => link.classList.remove('active'));
    ...
    function searchPapers() {
        ...
        if (activeFilters.has_full_text) params.set('has_full_text', true);
    ...
    function createPaperCard(paper) {
        ...
        if (paper.authors) {
            ...
            if (authorsList.length > 0) {
                ...
                authorsHtml += `<a href="?s=${encodeURIComponent(firstAuthor)}&post_type=mh_paper&search_field=author" class="mh-author-link">${escapeHtml(firstAuthor)}</a>`;
                ...
                if (lastAuthor && lastAuthor !== firstAuthor) {
                    ...
                    authorsHtml += `<a href="?s=${encodeURIComponent(lastAuthor)}&post_type=mh_paper&search_field=author" class="mh-author-link mh-last-author">${escapeHtml(lastAuthor)}</a>`;
                }
                authorsHtml += `</div>`;
            }
        }
