...
function mh_get_paper_tags($post_id = null) {
    ...
    // Get fluorophores from meta
    $fluorophores = json_decode(get_post_meta($post_id, '_mh_fluorophores', true), true) ?: array();
    foreach ($fluorophores as $fluor) {
        $tags[] = array(
            'name' => $fluor,
            'slug' => sanitize_title($fluor),
            'taxonomy' => 'fluorophore',
            'url' => null
        );
    }
    
    // Get sample prep from meta
    $sample_prep = json_decode(get_post_meta($post_id, '_mh_sample_prep', true), true) ?: array();
    foreach ($sample_prep as $prep) {
        $tags[] = array(
            'name' => $prep,
            'slug' => sanitize_title($prep),
            'taxonomy' => 'sample_prep',
            'url' => null
        );
    }
...
function mh_highlight_tags_in_text($text, $tags) {
    ...
    foreach ($tags as $tag) {
        ...
        $placeholder = '___TAG_' . md5($name) . '___';
        
        // Determine tag class based on taxonomy
        $class = 'mh-text-tag';
        switch ($tag['taxonomy']) {
            case 'mh_technique':
                $class .= ' mh-text-tag-technique';
                break;
            case 'mh_microscope':
                $class .= ' mh-text-tag-microscope';
                break;
            case 'mh_organism':
                $class .= ' mh-text-tag-organism';
                break;
            case 'mh_software':
                $class .= ' mh-text-tag-software';
                break;
            case 'fluorophore':
                $class .= ' mh-text-tag-fluorophore';
                break;
            case 'sample_prep':
                $class .= ' mh-text-tag-prep';
                break;
        }
        
        // Create the HTML replacement
        if ($tag['url'] && !is_wp_error($tag['url'])) {
            $replacement = '<a href="' . esc_url($tag['url']) . '" class="' . esc_attr($class) . '" title="View all papers with ' . esc_attr($name) . '">$0</a>';
        } else {
            $replacement = '<span class="' . esc_attr($class) . '">$0</span>';
        }
    ...
    // Apply replacements
    foreach ($replacements as $name => $data) {
 ...
 * Handles formats: [1], [1,2], [1-3], superscripts, etc.
 */
function mh_link_references_in_text($text, $references) {
        ...
        function($matches) use ($ref_lookup) {
            ...
            foreach ($parts as $part) {
                ...
                if (preg_match('/(\d+)\s*[-–]\s*(\d+)/', $part, $range)) {
                    ...
                    $end = intval($range[2]);
                    if ($end - $start <= 20) { // Reasonable range
                        $range_links = array();
                        for ($i = $start; $i <= $end; $i++) {
                            if (isset($ref_lookup[$i])) {
                                $range_links[] = '<a href="#ref-' . $i . '" class="mh-ref-link" title="Reference ' . $i . '">' . $i . '</a>';
                            } else {
                                $range_links[] = $i;
                            }
                        }
                        $linked_parts[] = implode('-', array($range_links[0], end($range_links)));
                    } else {
                        $linked_parts[] = $part;
                    }
                } else {
                    ...
                    if (isset($ref_lookup[$num])) {
                        $linked_parts[] = '<a href="#ref-' . $num . '" class="mh-ref-link" title="Reference ' . $num . '">' . $num . '</a>';
                    } else {
                        $linked_parts[] = $num;
                    }
                }
    ...
    $text = preg_replace_callback(
        ...
        function($matches) use ($ref_lookup, $superscripts) {
            $super = $matches[1];
            $num = '';
            $chars = preg_split('//u', $super, -1, PREG_SPLIT_NO_EMPTY);
            ...
            $num = intval($num);
            if ($num > 0 && isset($ref_lookup[$num])) {
                return '<a href="#ref-' . $num . '" class="mh-ref-link mh-ref-super" title="Reference ' . $num . '">' . $super . '</a>';
            }
            return $super;
        },
        $text
 ...
 */
function mh_parse_full_text_sections($full_text) {
    if (empty($full_text)) return array();
    
    // Section patterns with icons and order
    $section_config = array(
        'abstract' => array(
            'patterns' => array('/^abstract\s*$/mi', '/^summary\s*$/mi'),
            'icon' => '📝',
            'title' => 'Abstract',
            'order' => 1
        ),
        'introduction' => array(
            'patterns' => array('/^introduction\s*$/mi', '/^background\s*$/mi'),
            'icon' => '📖',
            'title' => 'Introduction',
            'order' => 2
        ),
        'methods' => array(
            'patterns' => array('/^(?:materials?\s*(?:and|&)?\s*)?methods?\s*$/mi', '/^experimental\s*(?:procedures?)?\s*$/mi'),
            'icon' => '🔬',
            'title' => 'Methods',
            'order' => 3
        ),
        'results' => array(
            'patterns' => array('/^results?\s*$/mi', '/^findings?\s*$/mi'),
            'icon' => '📊',
            'title' => 'Results',
            'order' => 4
        ),
        'discussion' => array(
            'patterns' => array('/^discussion\s*$/mi'),
            'icon' => '💭',
            'title' => 'Discussion',
            'order' => 5
        ),
        'conclusion' => array(
            'patterns' => array('/^conclusions?\s*$/mi'),
            'icon' => '✅',
            'title' => 'Conclusion',
            'order' => 6
        ),
        'acknowledgments' => array(
            'patterns' => array('/^acknowledge?ments?\s*$/mi'),
            'icon' => '🙏',
            'title' => 'Acknowledgments',
            'order' => 7
        ),
        'author_contributions' => array(
            'patterns' => array('/^author\s*contributions?\s*$/mi', '/^contributors?\s*$/mi'),
            'icon' => '👥',
            'title' => 'Author Contributions',
            'order' => 8
        ),
        'funding' => array(
            'patterns' => array('/^funding\s*$/mi', '/^financial\s*support\s*$/mi'),
            'icon' => '💰',
            'title' => 'Funding',
            'order' => 9
        ),
        'data_availability' => array(
            'patterns' => array('/^data\s*availability\s*$/mi', '/^data\s*access\s*$/mi'),
            'icon' => '📁',
            'title' => 'Data Availability',
            'order' => 10
        ),
        'conflicts' => array(
            'patterns' => array('/^(?:conflicts?\s*of\s*interest|competing\s*interests?)\s*$/mi'),
            'icon' => '⚖️',
            'title' => 'Conflicts of Interest',
            'order' => 11
        ),
        'supplementary' => array(
            'patterns' => array('/^supplement(?:ary|al)?\s*(?:materials?|information)?\s*$/mi'),
            'icon' => '📎',
            'title' => 'Supplementary Materials',
            'order' => 12
        ),
        'references' => array(
            'patterns' => array('/^references?\s*$/mi', '/^bibliography\s*$/mi', '/^literature\s*cited\s*$/mi'),
            'icon' => '📚',
            'title' => 'References',
            'order' => 99
        ),
    ...
    foreach ($section_config as $key => $config) {
        foreach ($config['patterns'] as $pattern) {
            if (preg_match_all($pattern, $full_text, $matches, PREG_OFFSET_CAPTURE)) {
    ...
    // If no sections found, return entire text as "content"
    if (empty($boundaries)) {
        return array(
            'content' => array(
                'key' => 'content',
    ...
    // Check if there's content before first section
    if ($boundaries[0]['pos'] > 100) {
        $pre_content = trim(substr($full_text, 0, $boundaries[0]['pos']));
        if (!empty($pre_content)) {
            $sections['preamble'] = array(
                'key' => 'preamble',
    ...
    for ($i = 0; $i < count($boundaries); $i++) {
        ...
        $start = $boundary['pos'] + strlen($boundary['match']);
        ...
        $content = trim(substr($full_text, $start, $end - $start));
        ...
        if (!empty($content)) {
            ...
            $sections[$key] = array(
                'key' => $boundary['key'],
                'title' => $boundary['config']['title'],
                'icon' => $boundary['config']['icon'],
                'content' => $content,
                'order' => $boundary['config']['order']
            );
        }
    }
...
function mh_display_full_text($post_id = null) {
    ...
    // Parse into sections
    $sections = mh_parse_full_text_sections($full_text);
    
    // Remove references section if we have parsed references (we'll show them separately)
    ...
    <section class="mh-paper-section mh-full-text-section">
        <div class="mh-full-text-header">
            ...
            <div class="mh-full-text-controls">
                ...
                <button type="button" class="mh-btn mh-btn-sm" id="mh-expand-all-sections">
                    <span class="expand-all">Expand All</span>
                    <span class="collapse-all" style="display:none;">Collapse All</span>
                </button>
            </div>
        </div>
        ...
        <!-- Table of Contents -->
        <nav class="mh-toc">
            <span class="mh-toc-label">Jump to:</span>
            <?php foreach ($sections as $key => $section) : ?>
                <a href="#section-<?php echo esc_attr($key); ?>" class="mh-toc-link">
                    <?php echo $section['icon']; ?> <?php echo esc_html($section['title']); ?>
                </a>
            <?php endforeach; ?>
            <?php if (!empty($references)) : ?>
                <a href="#section-references" class="mh-toc-link">📚 References</a>
            <?php endif; ?>
        </nav>
        ...
        <!-- Section Blocks -->
        <div class="mh-sections-container" id="mh-full-text-content">
            <?php foreach ($sections as $key => $section) : 
                // Process content
                ...
                $is_main_section = in_array($section['key'], array('abstract', 'introduction', 'methods', 'results', 'discussion', 'conclusion'));
            ?>
                <div class="mh-section-block <?php echo $is_main_section ? 'mh-section-expanded' : 'mh-section-collapsed'; ?>" 
                     id="section-<?php echo esc_attr($key); ?>"
                     data-section="<?php echo esc_attr($section['key']); ?>">
                    <div class="mh-section-header" onclick="MicroHub.toggleSection(this)">
                        <span class="mh-section-icon"><?php echo $section['icon']; ?></span>
                        <h3 class="mh-section-title"><?php echo esc_html($section['title']); ?></h3>
                        <span class="mh-section-toggle">
...
function mh_display_references($post_id = null) {
    ...
    ?>
    <section class="mh-paper-section mh-references-section" id="section-references">
        <div class="mh-section-block mh-section-expanded">
            <div class="mh-section-header" onclick="MicroHub.toggleSection(this)">
                <span class="mh-section-icon">📚</span>
                <h3 class="mh-section-title">References <span class="mh-ref-count">(<?php echo count($references); ?>)</span></h3>
                <span class="mh-section-toggle">
                    <span class="toggle-expand">▼</span>
                    <span class="toggle-collapse">▲</span>
                </span>
            ...
            <div class="mh-section-content">
                <ol class="mh-reference-list">
                    ...
                    ?>
                        <li id="ref-<?php echo esc_attr($num); ?>" class="mh-reference-item" value="<?php echo esc_attr($num); ?>">
                            <div class="mh-ref-content">
                                <span class="mh-ref-text"><?php echo esc_html($text); ?></span>
                                <div class="mh-ref-links">
                                    <?php if ($doi) : ?>
                                        <a href="https://doi.org/<?php echo esc_attr($doi); ?>" class="mh-ref-badge mh-ref-doi" target="_blank" rel="noopener">
                                            DOI
                                        </a>
                                    <?php endif; ?>
                                    <?php if ($pmid) : ?>
                                        <a href="https://pubmed.ncbi.nlm.nih.gov/<?php echo esc_attr($pmid); ?>" class="mh-ref-badge mh-ref-pmid" target="_blank" rel="noopener">
                                            PubMed
                                        </a>
                                    <?php endif; ?>
                                    <?php if ($url && !$doi && !$pmid) : ?>
                                        <a href="<?php echo esc_url($url); ?>" class="mh-ref-badge mh-ref-url" target="_blank" rel="noopener">
                                            Link
                                        </a>
                                    <?php endif; ?>
                                </div>
...
function mh_display_full_text($post_id = null) {
    ...
    // Highlight tags
    $processed_text = mh_highlight_tags_in_text($processed_text, $tags);
    
    // Link references
    $processed_text = mh_link_references_in_text($processed_text, $references);
    
    // Add section structure (detect common headers)
    $section_headers = array(
        'Abstract', 'Introduction', 'Background', 'Methods', 'Materials and Methods',
        'Results', 'Discussion', 'Conclusion', 'Conclusions', 'Acknowledgements',
        'Acknowledgments', 'References', 'Supplementary', 'Data Availability',
    ...
    foreach ($section_headers as $header) {
        $processed_text = preg_replace(
            ...
            '<h3 class="mh-section-header" id="section-' . sanitize_title($header) . '">$1</h3>',
            $processed_text
        );
    }
