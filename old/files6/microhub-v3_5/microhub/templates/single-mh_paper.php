            ...
            <div class="mh-paper-main">
                ...
                <?php if ($full_text) : 
                    ...
                    $section_config = array(
                        'abstract' => array('patterns' => array('/^abstract\s*$/mi', '/^summary\s*$/mi'), 'icon' => '📝', 'title' => 'Abstract', 'order' => 1),
                        'introduction' => array('patterns' => array('/^introduction\s*$/mi', '/^background\s*$/mi'), 'icon' => '📖', 'title' => 'Introduction', 'order' => 2),
                        'methods' => array('patterns' => array('/^(?:materials?\s*(?:and|&)?\s*)?methods?\s*$/mi', '/^experimental\s*(?:procedures?)?\s*$/mi'), 'icon' => '🔬', 'title' => 'Methods', 'order' => 3),
                        'results' => array('patterns' => array('/^results?\s*$/mi'), 'icon' => '📊', 'title' => 'Results', 'order' => 4),
                        'discussion' => array('patterns' => array('/^discussion\s*$/mi'), 'icon' => '💭', 'title' => 'Discussion', 'order' => 5),
                        'conclusion' => array('patterns' => array('/^conclusions?\s*$/mi'), 'icon' => '✅', 'title' => 'Conclusion', 'order' => 6),
                        'acknowledgments' => array('patterns' => array('/^acknowledge?ments?\s*$/mi'), 'icon' => '🙏', 'title' => 'Acknowledgments', 'order' => 7),
                        'references_section' => array('patterns' => array('/^references?\s*$/mi', '/^bibliography\s*$/mi'), 'icon' => '📚', 'title' => 'References', 'order' => 99),
                    );
                        ...
                        foreach ($config['patterns'] as $pattern) {
                            if (preg_match_all($pattern, $full_text, $matches, PREG_OFFSET_CAPTURE)) {
                                foreach ($matches[0] as $match) {
                                    $boundaries[] = array('key' => $key, 'pos' => $match[1], 'match' => $match[0], 'config' => $config);
                                }
                            }
                        }
                    }
                    usort($boundaries, function($a, $b) { return $a['pos'] - $b['pos']; });
                    ...
                    if (empty($boundaries)) {
                        $sections['content'] = array('key' => 'content', 'title' => 'Full Text', 'icon' => '📄', 'content' => $full_text, 'order' => 0);
                    } else {
                        if ($boundaries[0]['pos'] > 100) {
                            $pre = trim(substr($full_text, 0, $boundaries[0]['pos']));
                            if (!empty($pre)) {
                                $sections['preamble'] = array('key' => 'preamble', 'title' => 'Paper Info', 'icon' => '📄', 'content' => $pre, 'order' => 0);
                            }
                        }
                        for ($i = 0; $i < count($boundaries); $i++) {
                            $b = $boundaries[$i];
                            $start = $b['pos'] + strlen($b['match']);
                            $end = ($i + 1 < count($boundaries)) ? $boundaries[$i + 1]['pos'] : $text_length;
                            $content = trim(substr($full_text, $start, $end - $start));
                            if (!empty($content)) {
                                $k = isset($sections[$b['key']]) ? $b['key'] . '_' . $i : $b['key'];
                                $sections[$k] = array('key' => $b['key'], 'title' => $b['config']['title'], 'icon' => $b['config']['icon'], 'content' => $content, 'order' => $b['config']['order']);
                            }
                        }
                    }
                    
                    // Remove references section if we have parsed references
                    if (!empty($references)) { unset($sections['references_section']); }
                    uasort($sections, function($a, $b) { return $a['order'] - $b['order']; });
                    
                    // Get tags for highlighting
                    $all_tags = array();
                    foreach (array('mh_technique', 'mh_microscope', 'mh_organism', 'mh_software') as $tax) {
                        if (taxonomy_exists($tax)) {
                            $terms = get_the_terms($post_id, $tax);
                            if ($terms && !is_wp_error($terms)) {
                                foreach ($terms as $term) {
                                    $all_tags[] = array('name' => $term->name, 'url' => get_term_link($term), 'taxonomy' => $tax);
                                }
                            }
                        }
                    }
                    usort($all_tags, function($a, $b) { return strlen($b['name']) - strlen($a['name']); });
                ?>
                <section class="mh-paper-section mh-full-text-section">
                    ...
                    <nav class="mh-toc">
                        ...
                        <?php foreach ($sections as $skey => $sec) : ?>
                            <a href="#section-<?php echo esc_attr($skey); ?>" class="mh-toc-link"><?php echo $sec['icon']; ?> <?php echo esc_html($sec['title']); ?></a>
                        <?php endforeach; ?>
                    ...
                    <div class="mh-sections-container" id="mh-full-text-content">
                        <?php foreach ($sections as $skey => $sec) : 
                            ...
                            foreach ($all_tags as $tag) {
                                ...
                                $replacement = (!is_wp_error($tag['url'])) ? '<a href="' . esc_url($tag['url']) . '" class="' . esc_attr($class) . '">$1</a>' : '<span class="' . esc_attr($class) . '">$1</span>';
                                $content = preg_replace($pattern, $replacement, $content);
                            }
                            // Link references
                            if (!empty($references)) {
                                $content = preg_replace_callback('/\[(\d+(?:\s*[-–,]\s*\d+)*)\]/', function($m) {
                                    $nums = preg_split('/\s*,\s*/', $m[1]);
                                    $linked = array();
                                    foreach ($nums as $n) { $linked[] = '<a href="#ref-' . intval($n) . '" class="mh-ref-link">' . $n . '</a>'; }
                                    return '[' . implode(', ', $linked) . ']';
                                }, $content);
                            }
                            $is_main = in_array($sec['key'], array('abstract', 'introduction', 'methods', 'results', 'discussion', 'conclusion'));
                        ?>
                        <div class="mh-section-block <?php echo $is_main ? 'mh-section-expanded' : 'mh-section-collapsed'; ?>" id="section-<?php echo esc_attr($skey); ?>" data-section="<?php echo esc_attr($sec['key']); ?>">
                            <div class="mh-section-header" onclick="MicroHub.toggleSection(this)">
                                <span class="mh-section-icon"><?php echo $sec['icon']; ?></span>
                                <h3 class="mh-section-title"><?php echo esc_html($sec['title']); ?></h3>
                                <span class="mh-section-toggle"><span class="toggle-expand">▼</span><span class="toggle-collapse">▲</span></span>
                            </div>
                            <div class="mh-section-content"><?php echo $content; ?></div>
                        </div>
                        <?php endforeach; ?>
                    </div>
                </section>
                ...
                <section class="mh-paper-section mh-references-section" id="section-references">
                    <div class="mh-section-block mh-section-expanded" data-section="references">
                        <div class="mh-section-header" onclick="MicroHub.toggleSection(this)">
                                    ...
                                    <li id="ref-<?php echo esc_attr($num); ?>" class="mh-reference-item" value="<?php echo esc_attr($num); ?>">
                                        <div class="mh-ref-content">
                                            ...
                                            <div class="mh-ref-links">
                                                <?php if ($ref_doi) : ?>
                                                    <a href="https://doi.org/<?php echo esc_attr($ref_doi); ?>" class="mh-ref-badge mh-ref-doi" target="_blank" rel="noopener">DOI</a>
                                                <?php endif; ?>
                                                <?php if ($pmid) : ?>
                                                    <a href="https://pubmed.ncbi.nlm.nih.gov/<?php echo esc_attr($pmid); ?>" class="mh-ref-badge mh-ref-pmid" target="_blank" rel="noopener">PubMed</a>
                                                <?php endif; ?>
                                                <?php if ($url && !$ref_doi && !$pmid) : ?>
                                                    <a href="<?php echo esc_url($url); ?>" class="mh-ref-badge mh-ref-url" target="_blank" rel="noopener">Link</a>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </li>
