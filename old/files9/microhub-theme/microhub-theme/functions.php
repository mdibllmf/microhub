...
function mh_theme_register_rest_routes() {
    register_rest_route('microhub-theme/v1', '/papers', array(
        ...
        'args' => array(
            ...
            'facility' => array('type' => 'string', 'sanitize_callback' => 'sanitize_text_field'),
...
function mh_theme_search_papers($request) {
    ...
    if (!empty($facility) && taxonomy_exists('mh_facility')) {
        $term = $find_term('mh_facility', $facility);
        if ($term) {
            $tax_query[] = array(
...
function mh_get_facilities($limit = 10) {
    ...
    // First try to get from taxonomy (preferred method)
    ...
    $results = $wpdb->get_results($wpdb->prepare("
        SELECT meta_value as facility, COUNT(*) as count
    ...
    foreach ($results as $row) {
        $facilities[] = array(
            'name' => $row->facility,
            'slug' => sanitize_title($row->facility),
