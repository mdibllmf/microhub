 ...
 * 
 * Community discussions page with ability to view and post comments.
 * Uses mh_discussion custom post type for standalone discussions.
 */
get_header();
...
$categories = array(
    'techniques' => array('icon' => '🔬', 'name' => 'Techniques', 'desc' => 'Discuss imaging methods'),
    'software' => array('icon' => '💻', 'name' => 'Software', 'desc' => 'Analysis tools & plugins'),
    'protocols' => array('icon' => '📋', 'name' => 'Protocols', 'desc' => 'Methods & workflows'),
    'qa' => array('icon' => '❓', 'name' => 'Q&A', 'desc' => 'Ask the community'),
    'announcements' => array('icon' => '📢', 'name' => 'Announcements', 'desc' => 'News & updates'),
    'ideas' => array('icon' => '💡', 'name' => 'Ideas', 'desc' => 'Feature requests'),
);

// Get discussions (mh_discussion posts)
...
// Also get recent comments on papers
...
<div class="mh-container">
    ...
    <?php if ($discussion_submitted): ?>
        <div class="mh-success-message">
            <strong>✓ Discussion posted!</strong> Your discussion has been added to the community.
        </div>
    <?php endif; ?>
    ...
    <div class="mh-discussions-layout">
        ...
        <aside class="mh-discussions-sidebar">
                ...
                <ul class="mh-category-list">
                    <li class="mh-category-item <?php echo empty($category_filter) ? 'active' : ''; ?>">
                        <a href="<?php echo esc_url(remove_query_arg('category')); ?>">
                            <span class="mh-cat-icon">📚</span>
                            <div>
                                <strong>All Discussions</strong>
                                <small><?php echo count(get_posts(array('post_type' => 'mh_discussion', 'posts_per_page' => -1, 'fields' => 'ids'))); ?> topics</small>
                            </div>
                        </a>
                    </li>
            ...
            <div class="mh-sidebar-widget">
                ...
                <div class="mh-stats-list">
                    <div class="mh-stat-row">
                        ...
                        <span class="mh-stat-value"><?php echo count(get_posts(array('post_type' => 'mh_discussion', 'posts_per_page' => -1, 'fields' => 'ids'))); ?></span>
                    </div>
                    <div class="mh-stat-row">
            ...
            <div class="mh-sidebar-widget">
                ...
                <ul style="display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <li><a href="<?php echo esc_url(mh_get_page_url('protocols')); ?>">Protocols</a></li>
                    <li><a href="<?php echo esc_url(mh_get_page_url('contact')); ?>">Contact Us</a></li>
                </ul>
            </div>
        </aside>
        ...
        <div class="mh-discussions-main">
                    ...
                    <div class="mh-discussion-card">
                        ...
                        <div class="mh-discussion-content">
                            ...
                            <div class="mh-discussion-meta">
                                ...
                                <span class="mh-discussion-replies">💬 <?php echo $comment_count; ?> <?php echo $comment_count === 1 ? 'reply' : 'replies'; ?></span>
                            </div>
                        </div>
                    </div>
                    <?php endforeach; ?>
            ...
            <!-- Recent Paper Comments Section -->
            <?php if ($recent_paper_comments && !$category_filter): ?>
                <h2 style="margin: 40px 0 16px 0;">📄 Recent Paper Comments</h2>
...
.mh-discussion-card:hover {
    border-color: var(--primary, #58a6ff);
}
.mh-discussion-avatar {
    width: 48px;
    height: 48px;
    border-radius: 50%;
...
.mh-discussion-content {
    flex: 1;
    min-width: 0;
}
.mh-discussion-header {
    display: flex;
    align-items: center;
    gap: 10px;
    margin-bottom: 8px;
}
.mh-discussion-category {
    font-size: 1.25rem;
}
.mh-discussion-title {
    margin: 0;
...
.mh-discussion-title a {
    color: var(--text, #c9d1d9);
    text-decoration: none;
}
.mh-discussion-title a:hover {
    color: var(--primary, #58a6ff);
}
.mh-discussion-excerpt {
    color: var(--text-muted, #8b949e);
    font-size: 0.9rem;
    margin: 0 0 10px 0;
    line-height: 1.5;
...
.mh-category-item.active a {
    color: #fff;
}
.mh-category-item.active small {
    color: rgba(255,255,255,0.8);
}
.mh-cat-icon {
    font-size: 1.5rem;
}
.mh-category-item strong {
    display: block;
    font-size: 0.9rem;
}
.mh-category-item small {
    color: var(--text-muted, #8b949e);
    font-size: 0.8rem;
}

/* Paper Comments */
.mh-paper-comments-list {
    display: flex;
    flex-direction: column;
    gap: 12px;
}
.mh-paper-comment-card {
    display: flex;
    gap: 12px;
    padding: 16px;
    background: var(--bg-card, #161b22);
...
.mh-no-discussions .icon {
    font-size: 3rem;
    display: block;
    margin-bottom: 16px;
}
.mh-no-discussions h3 {
    margin: 0 0 8px 0;
    color: var(--text, #c9d1d9);
}
.mh-no-discussions p {
