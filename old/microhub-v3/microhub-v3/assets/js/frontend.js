 ...
 * Enhanced with Gemini AI, GitHub, discussions
 */
(function($) {
    'use strict';
    ...
    var MicroHub = {
        ...
        geminiKey: microhubData.geminiKey || '',
        perPage: 24,
        currentPage: 1,
        activeFilters: {},
        isLoading: false,
    ...
    MicroHub.initAIChat = function() {
        ...
        // Send message
        $(document).on('click', '#mh-ai-send', function() {
            self.sendAIMessage();
        });

        $(document).on('keypress', '#mh-ai-input', function(e) {
            if (e.which === 13) {
                self.sendAIMessage();
            }
        });
    ...
    /**
     * Send message to AI - Uses Gemini API
     */
    MicroHub.sendAIMessage = function() {
        var $input = $('#mh-ai-input');
        var message = $input.val().trim();
        if (!message) return;

        var $messages = $('#mh-ai-messages');

        // Add user message
        $messages.append('<div class="mh-ai-message user">' + this.escapeHtml(message) + '</div>');
        $input.val('');

        // Add typing indicator
        $messages.append('<div class="mh-ai-message assistant typing" id="ai-typing">Thinking...</div>');
        $messages.scrollTop($messages[0].scrollHeight);

        // Get paper context if viewing a paper
        var context = '';
        if (this.currentPaperContext) {
            context = 'The user is viewing a paper titled: "' + this.currentPaperContext + '"';
        }

        // Call Gemini API via WordPress REST endpoint
        var self = this;
        $.ajax({
            url: this.apiBase + '/ai-chat',
            method: 'POST',
            ...
            success: function(response) {
                $('#ai-typing').remove();
                var reply = response.reply || 'Sorry, I could not generate a response. Please try again.';
                
                // Format the response (convert markdown-style formatting)
                reply = self.formatAIResponse(reply);
                
                // If fallback response, show why
                if (response.fallback) {
                    var debugInfo = response.debug ? ' (' + response.debug + ')' : '';
                    reply += '<div class="mh-ai-setup-hint" style="margin-top:15px; padding:10px; background:#2d333b; border-radius:6px; font-size:0.85em;">' +
                        '⚠️ <strong>Using built-in responses</strong>' + debugInfo + '<br>' +
                        'To enable Gemini AI, add your API key in MicroHub → Settings.<br>' +
                        '<a href="https://aistudio.google.com/app/apikey" target="_blank" style="color:#58a6ff;">Get FREE API key →</a>' +
                        '</div>';
                }
                
                $messages.append('<div class="mh-ai-message assistant">' + reply + '</div>');
                $messages.scrollTop($messages[0].scrollHeight);
            },
            error: function(xhr, status, error) {
                $('#ai-typing').remove();
                console.log('AI Chat error:', status, error);
                
                var errorMsg = '<div class="mh-ai-error">Sorry, there was an error connecting to the AI. Please check your settings or try again later.</div>';
                $messages.append('<div class="mh-ai-message assistant">' + errorMsg + '</div>');
                $messages.scrollTop($messages[0].scrollHeight);
            }
        });
    };
