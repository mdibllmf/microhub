...
<section class="mh-search-section">
    <div class="mh-container">
        ...
        <div class="mh-quick-filters">
            ...
            <button type="button" class="mh-quick-btn" data-filter="has_figures">ðŸ“Š With Figures</button>
            <button type="button" class="mh-quick-btn" data-filter="has_github">ðŸ’» With Code</button>
...
(function() {
    ...
    function createProtocolCard(protocol) {
        ...
        if (protocol.microscopes?.length) {
            tagsHtml += `<span class="mh-card-tag microscope">ðŸ”¬ ${escapeHtml(protocol.microscopes[0])}</span>`;
        }
        ...
        if (protocol.microscope_brands?.length) {
            tagsHtml += `<span class="mh-card-tag brand">ðŸ­ ${escapeHtml(protocol.microscope_brands[0])}</span>`;
        }
        
        // Enrichment badges - same as homepage
        let enrichmentHtml = '';
        const items = [];
        if (protocol.protocols?.length) items.push(`<span class="mh-enrichment-item protocols">ðŸ“‹ ${protocol.protocols.length}</span>`);
        if (protocol.github_url) items.push(`<span class="mh-enrichment-item github">ðŸ’» Code</span>`);
        if (protocol.repositories?.length) items.push(`<span class="mh-enrichment-item">ðŸ’¾ Data</span>`);
        if (protocol.has_figures || protocol.figure_count > 0) items.push(`<span class="mh-enrichment-item figures">ðŸ“Š ${protocol.figure_count || 'Figs'}</span>`);
        if (protocol.fluorophores?.length) items.push(`<span class="mh-enrichment-item fluor" title="${escapeHtml(protocol.fluorophores.slice(0,3).join(', '))}">ðŸ§¬ ${protocol.fluorophores.length}</span>`);
        if (protocol.sample_preparation?.length) items.push(`<span class="mh-enrichment-item sample-prep" title="${escapeHtml(protocol.sample_preparation.slice(0,3).join(', '))}">ðŸ§« ${protocol.sample_preparation.length}</span>`);
        if (protocol.cell_lines?.length) items.push(`<span class="mh-enrichment-item cell-line" title="${escapeHtml(protocol.cell_lines.join(', '))}">ðŸ”¬ ${protocol.cell_lines.length}</span>`);
        if (protocol.rrids?.length) items.push(`<span class="mh-enrichment-item rrids">ðŸ·ï¸ ${protocol.rrids.length}</span>`);
