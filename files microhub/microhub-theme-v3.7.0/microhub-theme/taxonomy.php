...
<div class="mh-page-header">
    ...
    <p class="mh-page-subtitle"><?php echo esc_html($taxonomy->labels->singular_name); ?> â€¢ <?php echo $term->count; ?> papers</p>
    <?php 
    ...
    if ($term->taxonomy === 'mh_facility') {
        ...
        if ($location || $website) {
            ...
            if ($location) {
                echo '<span style="color: var(--text-muted);">ðŸ“ ' . esc_html($location) . '</span>';
            }
            if ($website) {
                echo '<a href="' . esc_url($website) . '" target="_blank" rel="noopener" style="color: var(--primary);">ðŸ”— Visit Website</a>';
            }
            echo '</div>';
        }
    }
        ...
        <article class="mh-card">
            ...
            <div class="mh-card-meta">
                ...
                <?php if ($meta['citations']): ?>
                    <span>ðŸ“Š <?php echo number_format($meta['citations']); ?></span>
                <?php endif; ?>
            </div>
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px;">
                <?php if (!empty($meta['protocols'])): ?>
                    <span style="font-size: 0.8rem; color: var(--secondary);">ðŸ“‹ <?php echo count($meta['protocols']); ?></span>
                <?php endif; ?>
                <?php if ($meta['github_url']): ?>
                    <span style="font-size: 0.8rem; color: var(--text-muted);">ðŸ’» Code</span>
                <?php endif; ?>
                <?php if (!empty($meta['repositories'])): ?>
                    <span style="font-size: 0.8rem; color: var(--text-muted);">ðŸ’¾ Data</span>
                <?php endif; ?>
                <?php if (!empty($meta['rrids'])): ?>
                    <span style="font-size: 0.8rem; color: #a371f7;">ðŸ·ï¸ <?php echo count($meta['rrids']); ?> RRID<?php echo count($meta['rrids']) > 1 ? 's' : ''; ?></span>
                <?php endif; ?>
            </div>
        </article>
        <?php endwhile; ?>
