...
<section class="mh-search-section">
    <div class="mh-container">
        ...
        <div class="mh-quick-filters">
            ...
            <button type="button" class="mh-quick-btn" data-filter="foundational">ðŸ† Foundational</button>
            <button type="button" class="mh-quick-btn" data-filter="high_impact">â­ High Impact</button>
            <button type="button" class="mh-quick-btn" data-filter="has_protocols">ðŸ“‹ Protocols <span id="count-protocols"><?php echo $stats['with_protocols']; ?></span></button>
            <button type="button" class="mh-quick-btn" data-filter="has_github">ðŸ’» GitHub <span id="count-github"><?php echo $stats['with_github']; ?></span></button>
            <button type="button" class="mh-quick-btn" data-filter="has_repositories">ðŸ’¾ Data <span id="count-repos"><?php echo $stats['with_repositories']; ?></span></button>
            <button type="button" class="mh-quick-btn" data-filter="has_figures">ðŸ“Š Figures <span id="count-figures"><?php echo $stats['with_figures']; ?></span></button>
            <button type="button" class="mh-quick-btn" data-filter="has_rrids">ðŸ·ï¸ RRIDs <span id="count-rrids"><?php echo $stats['with_rrids']; ?></span></button>
            <button type="button" id="mh-clear-filters" class="mh-clear-btn">âœ• Clear</button>
        </div>
    </div>
</section>
    ...
    <div class="mh-container">
        ...
        <div class="mh-main-layout">
            ...
            <aside class="mh-sidebar">
                ...
                <div class="mh-sidebar-widget">
                    <h3><span class="icon">ðŸ›ï¸</span> Research Institutions</h3>
                    ...
                    <ul class="mh-institution-list">
                        ...
                        <li class="mh-institution-item">
                            ...
                            <a href="<?php echo esc_url($institution['website']); ?>" class="institution-website" target="_blank" title="Visit institution website">ðŸ”—</a>
                            <?php endif; ?>
...
(function() {
    ...
    function createPaperCard(paper) {
        ...
        if (paper.microscopes?.length) {
            tagsHtml += `<span class="mh-card-tag microscope">ðŸ”¬ ${escapeHtml(paper.microscopes[0])}</span>`;
        }
        
        let enrichmentHtml = '';
        const items = [];
        if (paper.protocols?.length) items.push(`<span class="mh-enrichment-item protocols">ðŸ“‹ ${paper.protocols.length}</span>`);
        if (paper.github_url) items.push(`<span class="mh-enrichment-item github">ðŸ’» Code</span>`);
        if (paper.repositories?.length) items.push(`<span class="mh-enrichment-item">ðŸ’¾ Data</span>`);
        if (paper.has_figures || paper.figure_count > 0) items.push(`<span class="mh-enrichment-item figures">ðŸ“Š ${paper.figure_count || 'Figs'}</span>`);
        if (paper.fluorophores?.length) items.push(`<span class="mh-enrichment-item fluor" title="${escapeHtml(paper.fluorophores.slice(0,3).join(', '))}">ðŸ§¬ ${paper.fluorophores.length}</span>`);
        if (paper.sample_preparation?.length) items.push(`<span class="mh-enrichment-item sample-prep" title="${escapeHtml(paper.sample_preparation.slice(0,3).join(', '))}">ðŸ§« ${paper.sample_preparation.length}</span>`);
        if (paper.cell_lines?.length) items.push(`<span class="mh-enrichment-item cell-line" title="${escapeHtml(paper.cell_lines.join(', '))}">ðŸ”¬ ${paper.cell_lines.length}</span>`);
        if (paper.rrids?.length) items.push(`<span class="mh-enrichment-item rrids">ðŸ·ï¸ ${paper.rrids.length}</span>`);
    ...
    function renderPagination(total, totalPages) {
        ...
        let html = '';
        html += `<button ${currentPage === 1 ? 'disabled' : ''} data-page="${currentPage - 1}">â† Prev</button>`;
        ...
        html += `<button ${currentPage === totalPages ? 'disabled' : ''} data-page="${currentPage + 1}">Next â†’</button>`;
