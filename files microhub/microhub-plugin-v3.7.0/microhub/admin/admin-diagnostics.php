...
function microhub_diagnostic_page() {
    ...
    <div class="wrap">
        ...
        <h2>ðŸ”‘ Meta Keys Found in Database</h2>
        ...
        foreach ($all_meta_keys as $key) {
            ...
            echo esc_html($key) . " â†’ " . number_format($count) . " papers\n";
        }
        ?></pre>
        
        <hr>
        
        <h2>ðŸ“‹ Sample Papers with Protocols</h2>
                                ...
                                <?php foreach ($protocols as $proto) : ?>
                                    <div style="margin-bottom: 5px; padding: 5px; background: #f9f9f9; border-radius: 3px;">
                                        ...
                                        <?php if (!empty($proto['url'])) : ?>
                                            ...
                                            echo $url_check ? ' âœ…' : ' âŒ Invalid URL';
                                            ?>
                                        <?php else : ?>
                                            <span style="color: red;">âŒ No URL!</span>
                                        <?php endif; ?>
                                    </div>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <span style="color: orange;">âš ï¸ Invalid JSON: <?php echo esc_html(substr($paper->meta_value, 0, 100)); ?></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
        ...
        <h2>ðŸ’» Sample Papers with GitHub</h2>
            ...
            <table class="widefat striped" style="max-width: 800px;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <?php echo filter_var($paper->meta_value, FILTER_VALIDATE_URL) ? ' âœ…' : ' âŒ'; ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <h2>ðŸ’¾ Sample Papers with Data Repositories</h2>
        <?php 
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if (is_array($repos)) : ?>
                                <?php foreach ($repos as $repo) : ?>
                                    <div style="margin-bottom: 5px; padding: 5px; background: #f9f9f9; border-radius: 3px;">
                                        ...
                                        <?php if (!empty($repo['url'])) : ?>
                                            ...
                                            <?php echo filter_var($repo['url'], FILTER_VALIDATE_URL) ? ' âœ…' : ' âŒ Invalid'; ?>
                                        <?php else : ?>
                                            <span style="color: red;">âŒ No URL!</span>
                                        <?php endif; ?>
                                    </div>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <span style="color: orange;">âš ï¸ Invalid JSON</span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
        ...
        <h2>ðŸ›ï¸ Sample Papers with Facilities</h2>
        <?php 
        ...
        <h2>ðŸ“„ Sample Paper - All Meta Data</h2>
        <?php if ($sample_paper) : 
        ...
        <h2>ðŸ“ Expected JSON Format for Import</h2>
        ...
        <pre style="background: #1e1e1e; color: #d4d4d4; padding: 20px; border-radius: 8px; max-width: 800px; overflow-x: auto;">
{
  ...
  "facility": "Harvard Imaging Core"
        ...
        <h2>ðŸ”¬ v4.1 Meta Fields (for Theme Advanced Filters)</h2>
        <?php
        ...
        <table class="widefat striped" style="max-width: 800px;">
            ...
            <tbody>
                ...
                <tr style="<?php echo $data['count'] == 0 ? 'background: #f8d7da;' : ($data['count'] > 100 ? 'background: #d4edda;' : ''); ?>">
                    ...
                    <td>
                        <?php if ($data['count'] == 0) : ?>
                            âŒ Missing - Theme filters won't work
        ...
        <h3>ðŸ“Š Taxonomy vs Meta Field Comparison</h3>
                ...
                <tr style="<?php echo ($tax_count > 0 && $meta_count == 0) ? 'background: #fff3cd;' : ''; ?>">
                    ...
                    <td><?php echo number_format($meta_count); ?>
                        <?php if ($tax_count > 0 && $meta_count == 0) : ?>
                            <strong style="color: #856404;">â† Needs migration!</strong>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endforeach; ?>
        ...
        <h2>ðŸ”„ Migration Tool: Taxonomy â†’ Meta Fields</h2>
        ...
        if (isset($_POST['migrate_taxonomy_to_meta']) && check_admin_referer('microhub_migrate_nonce')) {
            ...
            echo '<div class="notice notice-success"><p>âœ… Migration complete! ' . esc_html($migrated) . ' papers updated.</p></div>';
        }
        ...
        if (isset($_POST['clear_theme_cache']) && check_admin_referer('microhub_cache_nonce')) {
            ...
            echo '<div class="notice notice-success"><p>âœ… Theme cache cleared!</p></div>';
        }
        ?>
        ...
        <form method="post" style="display: inline-block; margin-right: 20px;">
            ...
            <button type="submit" name="migrate_taxonomy_to_meta" class="button button-primary">
                ðŸ”„ Migrate Taxonomy Data to Meta Fields
            </button>
        </form>
        ...
        <h2>ðŸ§ª Test Single Paper Import</h2>
            ...
            <textarea name="test_paper_json" rows="10" style="width: 100%; max-width: 800px; font-family: monospace; font-size: 12px;" placeholder='Paste a single paper JSON object here, e.g.:
{
  ...
  "cell_lines": ["HeLa"]
            ...
            <button type="submit" name="test_import_paper" class="button button-primary">
                ðŸ§ª Test Import This Paper
            </button>
        </form>
        
        <hr>
        
        <h2>ðŸ”§ Quick Fix: Re-import with Update</h2>
        ...
        <ul>
            <li><a href="<?php echo rest_url('microhub/v1/papers?per_page=1'); ?>" target="_blank">ðŸ“„ Papers API</a></li>
            <li><a href="<?php echo rest_url('microhub/v1/enrichment-stats'); ?>" target="_blank">ðŸ“Š Enrichment Stats API</a></li>
            <li><a href="<?php echo rest_url('microhub/v1/protocols'); ?>" target="_blank">ðŸ“‹ Protocols API</a></li>
            <li><a href="<?php echo rest_url('microhub/v1/github-repos'); ?>" target="_blank">ðŸ’» GitHub Repos API</a></li>
            <li><a href="<?php echo rest_url('microhub/v1/data-repos'); ?>" target="_blank">ðŸ’¾ Data Repositories API</a></li>
            <li><a href="<?php echo rest_url('microhub/v1/facilities'); ?>" target="_blank">ðŸ›ï¸ Facilities API</a></li>
        </ul>
...
function microhub_test_single_import() {
    ...
    echo '<h3>ðŸ§ª Test Import Results</h3>';
    ...
    // Check if DOI exists
    if (empty($data['doi'])) {
        echo '<p style="color: red;">âŒ No DOI provided - cannot import.</p>';
        echo '</div>';
        return;
    }
    ...
    if ($post_id) {
        ...
        foreach ($taxonomies as $tax) {
            if (taxonomy_exists($tax)) {
                $terms = wp_get_object_terms($post_id, $tax, array('fields' => 'names'));
                $status = !empty($terms) && !is_wp_error($terms) ? 'âœ…' : 'âŒ';
                echo $status . ' ' . esc_html($tax) . ': ' . (is_array($terms) ? implode(', ', $terms) : '(none)') . "\n";
            } else {
                echo 'âŒ ' . esc_html($tax) . ': TAXONOMY NOT REGISTERED!\n';
            }
        }
        echo '</pre>';
