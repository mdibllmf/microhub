...
function mh_display_paper_links($meta = null) {
    ...
    // DOI - use stored URL if available, otherwise generate
    if ($meta['doi']) {
        $doi_url = !empty($meta['doi_url']) ? $meta['doi_url'] : 'https://doi.org/' . $meta['doi'];
        echo '<a href="' . esc_url($doi_url) . '" class="mh-btn mh-btn-doi" target="_blank">DOI</a>';
    }

    // PubMed - use stored URL if available, otherwise generate
    if ($meta['pubmed_id']) {
        $pubmed_url = !empty($meta['pubmed_url']) ? $meta['pubmed_url'] : 'https://pubmed.ncbi.nlm.nih.gov/' . $meta['pubmed_id'] . '/';
        echo '<a href="' . esc_url($pubmed_url) . '" class="mh-btn mh-btn-pubmed" target="_blank">PubMed</a>';
    }

    // PMC - use stored URL if available
    if (!empty($meta['pmc_id'])) {
        $pmc_url = !empty($meta['pmc_url']) ? $meta['pmc_url'] : 'https://www.ncbi.nlm.nih.gov/pmc/articles/' . $meta['pmc_id'] . '/';
        echo '<a href="' . esc_url($pmc_url) . '" class="mh-btn mh-btn-pmc" target="_blank">PMC</a>';
    }
