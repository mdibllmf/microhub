...
function microhub_single_template($template) {
    ...
    // Check theme first - if theme has the template, use it instead of plugin template
    if ($post && $post->post_type === 'mh_paper') {
        $theme_template = locate_template('single-mh_paper.php');
    ...
    if ($post && $post->post_type === 'mh_protocol') {
        $theme_template = locate_template('single-mh_protocol.php');
        if ($theme_template) return $theme_template;

        $custom = MICROHUB_PLUGIN_DIR . 'templates/single-mh_protocol.php';
        if (file_exists($custom)) return $custom;
    }
