#!/usr/bin/env python3
"""
MICROHUB JSON EXPORTER v5.1 - COMPREHENSIVE
============================================
...
- AFFILIATIONS (v5.0) - Raw author affiliation strings from PubMed
- INSTITUTIONS (v5.0 cleanup) - Extracted institution names  
...
CHANGES in v5.1:
- Added affiliations field (raw author affiliation strings from scraper v5)
- Added institutions field (normalized institution names from cleanup script v3)
- Added has_affiliations and has_institutions flags
- Statistics tracking for affiliations/institutions
"""
...
class ComprehensiveJsonExporter:
    ...
    def __init__(self, db_path: str = None):
        # Always use microhub.db in the script's directory if no path specified
        if db_path is None:
    ...
    def export(self, output_path: str = 'microhub_papers_v5.json',
        ...
        logger.info("=" * 60)
        logger.info("MICROHUB JSON EXPORTER v5.1 - COMPREHENSIVE")
        logger.info("=" * 60)
        ...
        # Check if GitHub tools tables exist
        tables_exist = False
        try:
            cursor_check = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('github_tools', 'paper_github_tools')")
            existing_tables = [row[0] for row in cursor_check.fetchall()]
            if 'github_tools' in existing_tables and 'paper_github_tools' in existing_tables:
                tables_exist = True
                logger.info("✓ GitHub tools tables found in database")
            else:
                logger.warning(f"⚠ GitHub tools tables missing. Found tables: {existing_tables}")
        except Exception as e:
            logger.warning(f"Could not check for GitHub tools tables: {e}")

        # Pre-load GitHub tool data for papers (if tables exist)
        github_tools_map = {}  # paper_id -> list of tool info
        github_tools_summary = []  # Global list of top tools
        if tables_exist:
            try:
                ...
                logger.warning(f"GitHub tools tables not found - skipping GitHub data: {e}")
                logger.warning("Run the scraper with GitHub extraction enabled to populate this data")
            except Exception as e:
        ...
        stats = {
            ...
            'with_cell_lines': 0,
            'with_staining': 0,
            'with_affiliations': 0,      # NEW in v5.1
            'with_institutions': 0,      # NEW in v5.1
            'with_reagent_suppliers': 0, # NEW in v5.2
            'with_general_software': 0,  # NEW in v5.2
            'total_citations': 0,
            'total_figures': 0,
            'total_protocols': 0,
            ...
            'total_rors': 0,
            'total_rrids': 0,
            'total_affiliations': 0,     # NEW in v5.1
            'total_institutions': 0,     # NEW in v5.1
            'from_methods': 0,
        ...
        for row in cursor:
            ...
            paper = {
                ...
                # === AFFILIATIONS & INSTITUTIONS (v5.0/v5.1) ===
                # Raw author affiliation strings from PubMed (scraper v5)
                'affiliations': affiliations,
                # Normalized institution names (cleanup script v3)
                ...
                # === REAGENT SUPPLIERS (v5.2 - separated from microscope brands) ===
                ...
                'software': software if software else (image_analysis_software + image_acquisition_software + general_software),
                ...
                'has_affiliations': bool(affiliations) or bool(self.safe_get(row_dict, 'has_affiliations')),  # v5.1
                'has_institutions': bool(institutions) or bool(self.safe_get(row_dict, 'has_institutions')),  # v5.1
            ...
            # Affiliations & Institutions stats (v5.1)
            if affiliations:
            ...
            # Reagent suppliers & General software stats (v5.2)
            if reagent_suppliers:
        ...
        # Export GitHub tools summary if available
        tools_filename = f"{base_name}_github_tools.json"
        if github_tools_summary:
            ...
            created_files.append(tools_filename)
            logger.info(f"✓ Saved GitHub tools summary: {tools_filename} ({len(github_tools_summary)} tools)")
        else:
            logger.warning(f"⚠ No GitHub tools found in database - {tools_filename} not created")
            logger.warning("  Make sure the scraper has run with GitHub extraction enabled")
        ...
        logger.info("")
        logger.info("AFFILIATIONS & INSTITUTIONS (v5.1):")
        logger.info(f"  With affiliations: {stats['with_affiliations']:,} ({stats['total_affiliations']:,} total)")
        logger.info(f"  With institutions: {stats['with_institutions']:,} ({stats['total_institutions']:,} total)")
        logger.info("")
...
def main():
    # Default to microhub.db in the script's directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    default_db = os.path.join(script_dir, 'microhub.db')
    default_output = os.path.join(script_dir, 'microhub_papers_v5.json')
    
    parser = argparse.ArgumentParser(description='Export papers to JSON v5.1 - Comprehensive with affiliations/institutions')
    parser.add_argument('--db', default=default_db, help='Database path (default: microhub.db in script directory)')
    parser.add_argument('--output', '-o', default=default_output, help='Output filename (default: in script directory)')
