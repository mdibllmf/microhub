...
class MicroHub_AI_Knowledge {
    ...
    public function identify_entities($text, $category = '') {
        ...
        // Words that are too common/ambiguous to match on their own
        // These should only match as part of full entity names or specific keywords
        $ambiguous_words = array(
            'prior', 'applied', 'advanced', 'digital', 'universal', 'general',
            'national', 'international', 'american', 'european', 'scientific',
            'imaging', 'optical', 'precision', 'micro', 'nano', 'bio', 'cell',
            'life', 'medical', 'research', 'analysis', 'systems', 'technologies'
        );
        ...
        foreach ($entries as $entry) {
            ...
            // Check full title as entity name (exact phrase match - HIGH confidence)
            $title_lower = strtolower($entry->title);
            if (stripos($text_lower, $title_lower) !== false) {
                $matches += 10;
                $matched_terms[] = $entry->title;
            }
            
            // Check keywords (must be multi-word or specific terms)
            foreach ($keywords as $keyword) {
                // Skip single ambiguous words in keywords
                if (in_array($keyword, $ambiguous_words)) {
                    continue;
                }
                // Require longer keywords or multi-word phrases
                if (strlen($keyword) >= 5 && stripos($text_lower, $keyword) !== false) {
                    $matches += 3;
                    $matched_terms[] = $keyword;
                }
            }
            
            // DO NOT check individual title words - this causes false positives
            // like "prior" matching "Prior Scientific"
            
            // Only identify if we have a strong match (full title or specific keywords)
            if ($matches >= 5) {
                $identified[] = array(
                    'entity' => $entry->title,
                    'category' => $entry->category,
